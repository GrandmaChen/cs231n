...
class TwoLayerNet(object):
    ...
    # input_size=4, hidden_size=10, output_size=num_classes=3
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # W1.shape = (4, 10)  b1.shape = (10,)
        W2, b2 = self.params['W2'], self.params['b2']
        # W2.shape = (4, 10)  b2.shape = (10,)
        N, D = X.shape
        # N = 5  D = 4
        ...
        scores_correct = scores[range(N), y]  # shape = (5,)

        margin = -np.log(np.exp(scores_correct) / np.exp(scores).sum(axis=1))
        loss = margin.mean() + reg * np.sum(W1 * W1) + reg * np.sum(W2 * W2)
        ...
        dW2 = scores_hidden_layer.T.dot(dW2_factors)  # shape = (10, 3)
        dW2 /= N
        dW2 += 2 * reg * W2

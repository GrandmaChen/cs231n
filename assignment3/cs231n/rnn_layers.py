...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        # Since an input x is of shape (N, D), so in order to extract t-th input x
        # from (N, T, D) we need to select x[:, t, :] (equals to x[:, t])
        xt = x[:, t]

        curr_h, cache[t] = rnn_step_forward(xt, prev_h, Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    """
    dx, dh0, dWxt, dWh, db = None, None, None, None, None
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # dtanh(x) = 1 - tanh(x)^2
    dtanhc_t = dnext_h * o
    dnext_c += dtanhc_t * (1 - np.tanh(next_c) ** 2)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        # Get current h & c using t-th x and lstm step forward function
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        dxt, dprev_h, dprev_c, dWxt, dWht, dbt = lstm_step_backward(dcurr_h + dprev_h, dcurr_c, cache[t])
